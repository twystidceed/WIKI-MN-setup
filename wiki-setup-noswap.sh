#!/bin/bash
# WikiCoin Masternode Setup Script V1.6 for Ubuntu 16.04 LTS
#
...
echo -e "${YELLOW}Wiki Masternode Setup Script V1 for Ubuntu 16.04 LTS${NC}"
...
rpcpassword=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
 ...
 #Installing Daemon
 cd ~
wget https://github.com/Altcoinwiki/source/releases/download/v1.1.0/WikiCoin-1.1.0-x86_64-pc-linux-gnu.zip
unzip WikiCoin-1.1.0-x86_64-pc-linux-gnu.zip -d ~/WIKI-MN-setup
rm -rf WikiCoin-1.1.0-x86_64-pc-linux-gnu.zip

 
 stop_daemon
 
 # Deploy binaries to /usr/bin
 sudo rm ~/WIKI-MN-setup/WikiCoin-1.1.0-x86_64-pc-linux-gnu/wiki-qt
 sudo cp ~/WIKI-MN-setup/WikiCoin-1.1.0-x86_64-pc-linux-gnu/wiki* /usr/bin/
 sudo chmod 755 -R ~/WIKI-MN-setup
 sudo chmod 755 /usr/bin/wiki*
 
 # Deploy masternode monitoring script
 cp ~/WIKI-MN-setup/wikimon.sh /usr/local/bin/
 sudo chmod 711 /usr/local/bin/wikimon.sh
...
Currently your masternode is syncing with the Wiki network...
...
echo -e "${NC}-------------------------------------------------
NOTE: To edit wiki.conf, first stop the wikid daemon,
...
========================================================================
To view Wiki debug log showing all MN network activity in realtime:
...
sync status etc. in real-time, run the wikimon.sh script:
                 ${YELLOW}wikimon.sh${NC}
or just type 'node' and hit <TAB> to autocomplete script name.
========================================================================
Enjoy your Wiki Masternode and thanks for using this setup script!
